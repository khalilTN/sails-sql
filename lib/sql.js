...
var sql = {
      ...
      queryObject.criteriasByParent.forEach(function (criteria, index) {
        if(!unionQuery){
          unionQuery = client.select('*').from(function () {
            self.select(this, queryObject.collectionName, queryObject.schema, criteria, true).as('parent_'+(index + 1));
          });
        }
        else{
          unionQuery = unionQuery.union(function(){
    ...
    manyToManyUnion: function (client, queryObject) {
      ...
      queryObject.parentIds.forEach(function (id, index) {
        if (!unionQuery) {
          unionQuery = client.select('*').from(function () {
            var req = self.select(this, queryObject.childCollection, queryObject.schema, queryObject.criteria.criteria, true).as('parent_' + (index + 1));
