...
var sql = {
    ...
    select: function (client, collectionName, options, sqlMode, schema) {
        var query;
        if (!sqlMode)
        ...
        query = this.processPopulations(query, options, schema);
        if (!sqlMode)
    ...
    processPopulations: function (query, criteria, schema) {
        var self = this;
        ...
        _.keys(criteria.instructions).forEach(function (population) {
            ...
            if (populationObject.strategy.strategy === 1) {
                ...
                var parentDefinition = self.getDefinitionBytableName(schema, infos.parent).attributes;
    ...
    fetchOneToManyChildren: function (client, queryObject) {
      ...
      if(criteria.sort){
        if(!criteria.select){
          criteria.select = _.map(queryObject.childColumns, function(column){
      ...
      queryObject.parentsIds.forEach(function (id) {
        ...
        if(!unionQuery){
          unionQuery = client.union(function(){
            self.select(this, queryObject.childCollection, criteria, true);
          }, true);
        }
        else{
    ...
    fetchManyToManyChildren: function (client, queryObject) {
      ...
      function query(context, id){
        var req = self.select(context, queryObject.childCollection, criteria, true);
