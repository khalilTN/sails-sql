...
OracleDialect.prototype.manyToManyUnion = function (client, queryObject) {
  if (!queryObject || queryObject.parentIds.length === 0)
    return null;
  if (!queryObject.criteria.criteria.select)
    queryObject.criteria.criteria.select = [];
  ...
  var unionQuery;
  queryObject.parentIds.forEach(function (id) {
    if (!unionQuery) {
      unionQuery = client.select('*').from(function () {
        var query = SQL.select(this, queryObject.childCollection, queryObject.schema, queryObject.criteria.criteria, true);
        query.innerJoin(queryObject.jonctionCollection, queryObject.jonctionCollection + '.' + queryObject.jonctionChildFK, queryObject.childCollection + '.' + queryObject.childPK);
        query.whereIn(queryObject.childCollection + '.' + queryObject.childPK, function () {
        ...
        query.where(queryObject.jonctionCollection + '.' + queryObject.jonctionParentFK, id);
      });
    }
    else {
      unionQuery.unionAll(function () {
